...
============*/
/* HIGHLIGHT: CHANGE ====================
   An overloaded print function was implemented to print to output stream object (other one prints to screen).
==================== HIGHLIGHT: CHANGE */
#ifndef CURRENCY_H
#define CURRENCY_H
...
bool Currency::isGreater(Currency* otherObject) const {
  ...
  else if((*(this->whole) == *(otherObject->whole)) && (*(this->fraction) > *(otherObject->fraction))) {
    result = true;
    // std::cout << "true2\n";
  }
  return result;
...
/*
This function prints the value of the calling object (whole added to fraction which is divided by 100) to 2 decimal places then the currency type to the screen.
Pre: 
Post: The value is printed to a precision of 2 decimal points and the currency type is printed to the screen.
Return: 
*/
void Currency::print() {
...
/* HIGHLIGHT: CHANGE ====================
    An overloaded print function was implemented to print to output stream object (other one prints to screen).
==================== HIGHLIGHT: CHANGE
This function prints the value of the calling object (whole added to fraction which is divided by 100) to 2 decimal places then the currency type to the output stream object.
Pre: 
Post: The value is printed to a precision of 2 decimal points and the currency type is printed to the output stream object.
Return: 
*/
void Currency::print(ostream& out) {
  out << std::fixed << std::setprecision(2);
  out << getWhole() + (getFraction()/100.0) << " " << getType() << "\t";
}
